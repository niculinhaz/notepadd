          ...
          <>
            <View style={styles.editorPane}>
              <View style={styles.inputsContainer}>
                <TextInput style={styles.inputTitle} placeholder="Título" placeholderTextColor="#555" value={title} onChangeText={setTitle} />
                <TextInput style={styles.inputTag} placeholder="#tag" placeholderTextColor="#555" value={tag} onChangeText={setTag} />
              </View>
              <TextInput 
                style={styles.textArea} multiline placeholder="Digite aqui..." placeholderTextColor="#444" 
                value={content} onChangeText={setContent} 
                onSelectionChange={(e) => setSelection(e.nativeEvent.selection)} autoFocus 
              />
            </View>
            <View style={styles.toolbar}>
               <ScrollView horizontal contentContainerStyle={styles.toolbarContent} keyboardShouldPersistTaps="always">
                  {['**', '*', '# ', '## ', '\n- ', '`'].map((sym, i) => (
                    <TouchableOpacity key={i} style={styles.toolBtn} onPress={() => handleFormat(sym, sym.trim() === '**' || sym.trim() === '*' || sym.trim() === '`' ? sym : '')}>
